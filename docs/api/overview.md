...
| Header | 必填 | 说明 |
| --- | --- | --- |
| `X-Admin-Key` | 是 | 初始化时设置的管理员 Key（与后台登录相同） |
...
| 场景 | 上限（每 IP / 小时） | 说明 |
| --- | --- | --- |
| 初始化管理员 Key | `20` | 首次 `/auth/setup` |
| 登录失败 | `60` | 后台登录 Key 错误 |
| 公开接口鉴权失败 | `120` | `X-Admin-Key` 缺失或错误 |
| 共享访问 | `2000` | 共享详情 / 附件 / 共享账户 API（见共享文档） |

鉴权失败会计入公开接口限流；鉴权成功不会写入限流计数。限流不可在配置中心修改。

...
共享入口默认跟随后台入口。后台入口为任意时，会使用当前请求的入口生成共享地址。也可以在 **配置中心** 指定独立共享入口。
...
限流为系统**内置**（按 IP / 小时），不在配置中心修改。

创建共享属于公开 API：鉴权成功不计入限流；鉴权失败计入「公开接口鉴权失败」限流。

`/mail/:token` 和 `/account/:token` 是公开页面入口，与附件下载、共享 API 一样计入「共享访问」限流。

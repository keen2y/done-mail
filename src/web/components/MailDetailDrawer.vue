  ...
  <aside class="mc-detail-drawer" :class="{ 'mc-loading-surface': loading }" :aria-busy="loading ? 'true' : 'false'" aria-label="邮件详情抽屉">
    ...
    <template v-if="title">
      <div class="mc-detail-title">
        <button type="button" class="mc-detail-back" aria-label="返回邮件列表" title="返回列表" @click="emit('close')">
          <McIcon name="left" :size="20" />
        </button>
        ...
        <div ref="actionRef" class="mc-detail-menu-wrap">
          <button
            v-if="hasActions"
            type="button"
            class="mc-icon-action mc-detail-menu-trigger"
            :aria-label="actionLabel || '邮件操作'"
          ...
          <button type="button" class="mc-icon-action mc-detail-close" aria-label="关闭邮件详情" title="关闭" @click="emit('close')">

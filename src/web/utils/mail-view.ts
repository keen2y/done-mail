...
function shanghaiYmd(date: Date) {
  return date.toLocaleDateString('en-CA', { timeZone: DISPLAY_TZ });
}

function shanghaiMonthDay(date: Date) {
...
export function formatTime(value: string) {
  ...
  const time = date.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: DISPLAY_TZ });
  const day = shanghaiYmd(date);
  const today = shanghaiYmd(new Date());
  if (day === today) return time;
  const yesterdayDate = new Date();
  yesterdayDate.setDate(yesterdayDate.getDate() - 1);
  if (day === shanghaiYmd(yesterdayDate)) return `昨天 ${time}`;
  return `${shanghaiMonthDay(date)} ${time}`;
}
...
export function formatFullTime(value: string) {
  ...
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return value;
  const day = date.toLocaleDateString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', timeZone: DISPLAY_TZ }).replace(/\//g, '-');
  const time = date.toLocaleTimeString('zh-CN', { hour12: false, timeZone: DISPLAY_TZ });
  return `${day} ${time}`;
}
...
<style>
  ...
  /* Constrain overflow without forcing every CTA table to full width */

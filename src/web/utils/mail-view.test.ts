...
describe('mail view', () => {
  ...
  it('不强制表格 100% 宽，避免 CTA 按钮被拉成整行', () => {
    const srcdoc = mailHtmlSrcdoc({
      htmlBody:
        '<table width="200" style="width:200px;background:#f6821f"><tr><td><a href="https://example.com/verify" style="color:#fff;padding:12px 24px">Verify</a></td></tr></table>'
    });

    expect(srcdoc).toMatch(/table\s*\{\s*max-width:\s*100%\s*!important/);
    expect(srcdoc).not.toMatch(/table\s*\{[^}]*[^a-z-]width:\s*100%\s*!important/);
    expect(srcdoc).toContain('style="width:200px;background:#f6821f"');
    expect(srcdoc).toContain('style="color:#fff;padding:12px 24px"');
  });
});

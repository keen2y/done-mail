...
async function loadShares(cursor = list.currentCursor(), force = false) {
  try {
    ...
    loaded.value = true;
    ElMessage.error(apiErrorMessage(error, '共享账户加载失败'));
  }
}
      ...
      <div class="mc-panel-head mc-panel-head--tools">
        ...
        <div class="mc-page-actions mc-panel-head-tools">
          <label class="mc-search mc-search--table mc-panel-head-search">
            ...
            <input v-model.trim="query.keyword" type="search" placeholder="搜索邮箱账户" @keyup.enter="applySearch" />
          </label>
    ...
    >
      <template #default>
        <el-form-item label="邮箱账户">
          <el-input v-model.trim="draft.mailbox" placeholder="name@example.com" />
        </el-form-item>
      </template>
    </ShareExpiryDialog>

...
async function openAddDomainDialog() {
  ...
  if (!cloudflareConfigured()) {
    try {
      ...
      // keep previous settings snapshot
    }
  }

  if (!cloudflareConfigured()) {
    ElMessage.warning({
      message: '请先到「配置中心」完成 Cloudflare 配置',
      duration: 3200
    });
    return;
  }
...
async function removeLocalOne(row: DomainRow) {
  ...
  const confirmed = await confirmDialog({
    title: '移除本地记录',
    message: `确认移除「${row.name}」的本地记录？不会修改 Cloudflare 侧配置。`,
    confirmText: '移除',
    intent: 'danger'
  });
  if (!confirmed) return;
...
function clearRootSelection(event: PointerEvent) {
  ...
  if (
    target?.closest(
      '.el-table__row, .mc-table-icon-action, .mc-floating-menu, .mc-page-actions, .el-overlay, .mc-dialog'
    )
  ) {
    return;
  }
...
onActivated(() => {
  domainsActive = true;
  void Promise.all([loadDomains(rootCursor(), true), loadSettings()]).then(syncDomainStatusPolling);
});
      ...
      <div class="mc-panel-head mc-panel-head--tools">
        ...
        <div class="mc-page-actions mc-panel-head-tools">
          <label class="mc-search mc-search--table mc-panel-head-search">
            ...
            <input v-model.trim="activeQuery.keyword" type="search" :placeholder="activeSearchPlaceholder" @keyup.enter="applySearch" />
          </label>

...
function currentDraftFingerprint() {
  try {
    return JSON.stringify(serializePolicy(draft, editorMode.value === 'edit'));
  } catch {
    return '';
  }
}
...
function markDraftClean() {
  draftFingerprint.value = currentDraftFingerprint();
}

function isDraftDirty() {
  if (editorMode.value === 'idle') return false;
  return currentDraftFingerprint() !== draftFingerprint.value;
}
...
async function openEditPolicy(policy: MailPolicy) {
  if (selectedPolicyId.value === policy.id && editorMode.value === 'edit') {
    actionMenuOpen.value = false;
    return;
  }
  if (!(await confirmDiscardDraft())) return;
...
async function openPolicyFromList(policy: PolicyListItem) {
  if (editorMode.value === 'create' && policy.id === draft.id) {
    selectedPolicyId.value = draft.id;
    return;
  }
  await openEditPolicy(policy as MailPolicy);
}
...
async function loadPolicies(options: LoadPolicyOptions = {}, force = false) {
  try {
    ...
    policiesLoaded.value = true;
    ElMessage.error(apiErrorMessage(error, '邮件策略加载失败'));
  }
}
      ...
      <div class="mc-panel-head mc-panel-head--tools">
        ...
        <div class="mc-page-actions mc-panel-head-tools">
          <label class="mc-search mc-search--table mc-panel-head-search">
            ...
            <input v-model.trim="query.keyword" type="search" placeholder="搜索策略、条件、动作" @keyup.enter="applySearch" />
          </label>

...
import type { AppConfig, CloudflareConfig, Env, PublicCloudflareConfig, SystemConfig } from './types';
...
export async function getSystemConfig(env: Env): Promise<SystemConfig> {
  ...
  const value = {
    ...
    mailRetentionDays: Math.max(Math.floor(Number(retentionDays) || 30), 1),
...
export async function getAppConfig(env: Env): Promise<AppConfig> {
  const [cloudflare, system] = await Promise.all([getCloudflareConfig(env), getSystemConfig(env)]);
...
export async function buildSettingsUpdate(env: Env, input: Partial<AppConfig> & { cloudflare?: Partial<CloudflareConfig> }) {
  ...
  const system: SystemConfig = {
    ...
    mailRetentionDays: Math.max(Math.floor(Number(input.system?.mailRetentionDays ?? current.system.mailRetentionDays) || 30), 1),
...
export async function saveSettingsUpdate(env: Env, config: AppConfig) {
  await Promise.all([env.KV.put(CLOUDFLARE_KEY, JSON.stringify(config.cloudflare)), env.KV.put(SYSTEM_KEY, JSON.stringify(config.system))]);

...
describe('schema migrations', () => {
  it('空 D1 首次访问会创建业务表和迁移记录', async () => {
    ...
    expect(calls.some((call) => call.sql.includes('CREATE TABLE IF NOT EXISTS mail_bodies') && call.sql.includes('text_body') && call.sql.includes('html_body'))).toBe(true);
    expect(calls.some((call) => call.sql.includes('CREATE VIRTUAL TABLE IF NOT EXISTS mail_content_fts'))).toBe(true);
    expect(calls.some((call) => call.sql.includes('rate_limits'))).toBe(false);
    ...
    expect(calls.some((call) => call.sql.includes('mail_body_chunks'))).toBe(false);
    expect(calls.some((call) => call.sql.includes('mail_safe_bodies'))).toBe(false);
    expect(calls.some((call) => call.sql.includes('sent_mails'))).toBe(false);

...
import { clearAuthConfigCache, getAuthConfig } from '../config';
...
authRoutes.post('/setup', async (c) => {
  const limited = await consumeRateLimit(c.env, 'setup', rateLimitIdentity(c), getRateLimit('setup'));
  if (limited) {
    return apiFail(c, '初始化尝试过于频繁，请稍后再试', 429, 'rate_limited');
  }
...
authRoutes.post('/login', async (c) => {
  ...
  if (!(await verifyAdminKeyConfig(authConfig, adminKey))) {
    const limited = await consumeRateLimit(c.env, 'login', rateLimitIdentity(c), getRateLimit('login'));
    await clearAuthSession(c);
    if (limited) {

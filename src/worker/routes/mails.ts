...
import { getLatestMailRow, listMailRows, mapMailRow, pushMailFilters, type MailRow } from '../mail-list';
...
function publicMailMatchSql(subject: string, content: string, params: string[]) {
  ...
  if (subjectQuery && contentQuery) {
    params.push(subjectQuery, contentQuery);
    return `SELECT mail_id FROM mails_fts WHERE mails_fts MATCH ?
INTERSECT
SELECT DISTINCT mail_id FROM mail_content_fts WHERE mail_content_fts MATCH ?`;
  ...
  if (contentQuery) {
    ...
    return `SELECT DISTINCT mail_id FROM mail_content_fts WHERE mail_content_fts MATCH ?`;
  }
  return '';
}
...
async function listPublicMailRows(env: Env, query: PublicMailListQuery) {
  ...
  if (pageItems.length > 0) {
    ...
    const bodiesPromise = listMailBodies(env, mailIds);
...
async function listPublicMails(c: AppContext) {
  // Default small page (bodies always included); hard cap keeps payload bounded.
  const perPage = pageSize(c.req.query('limit'), 3, 1, 20);

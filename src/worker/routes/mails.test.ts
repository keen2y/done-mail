  ...
  const prepare = vi.fn((sql: string) => {
    const statement = {
      ...
      all: vi.fn(async () => {
        ...
        if (sql.includes('FROM mail_bodies')) return bodies;
        return { results: rows };
      })
    };
    return statement;
...
describe('public mails route', () => {
  ...
  it('内部关键词搜索使用单条 AND FTS，跨主题和正文 UNION', async () => {
    const { env, calls } = createEnv();
    ...
    expect(listCall.sql).toContain('mails_fts MATCH ?');
    expect(listCall.sql).toContain('mail_content_fts MATCH ?');
    expect(listCall.sql).toContain('UNION');
    expect(listCall.sql).not.toContain('INTERSECT');
    expect(listCall.sql).not.toContain('term_0');
    expect(listCall.params).toHaveLength(3);
    expect(String(listCall.params[0])).toContain('"invoice"*');
    expect(String(listCall.params[0])).toContain(' AND ');
    expect(String(listCall.params[0])).toContain('"完整"*');
    expect(String(listCall.params[1])).toBe(String(listCall.params[0]));
  });
  ...
  it('公开邮件列表默认返回正文并限制单页大小', async () => {
    ...
    // limit=100 is clamped to max 20
    expect(body.pagination).toMatchObject({ limit: 20, hasMore: false, nextCursor: '' });
    ...
    expect(listCall.params).toEqual([21]);
  });

  it('公开邮件列表未传 limit 时默认 3', async () => {
    const { env, calls } = createEnv();
    const response = await publicMailsRoutes.fetch(new Request('https://example.com/'), env);
    const body = await response.json() as { pagination: { limit: number } };
    expect(response.status).toBe(200);
    expect(body.pagination.limit).toBe(3);
    expect(calls[0]?.params).toEqual([4]);
  });

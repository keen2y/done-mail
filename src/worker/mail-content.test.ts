...
describe('mail-content', () => {
  ...
  it('清理邮件 HTML 中的危险标签和跳转协议，保留安全样式', () => {
    const html =
      '<a href="javascript:alert(1)" onclick="x()" style="color:red;background:#f6821f;padding:12px 24px">打开</a><iframe src="https://x.test"></iframe><svg onload="x()"></svg><img src="data:text/html;base64,xx"><span style="color:expression(alert(1));background:url(javascript:alert(1))">x</span>';
    const clean = sanitizeMailHtml(html);
    expect(clean).toContain('href="#"');
    expect(clean).not.toContain('onclick');
    expect(clean).toContain('style="color:red;background:#f6821f;padding:12px 24px"');
    expect(clean).not.toContain('expression');
    expect(clean).not.toContain('url(javascript');

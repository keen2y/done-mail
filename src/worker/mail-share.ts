...
export async function createShare(env: Env, rawInput: ShareInput, requestUrl = '') {
  ...
  try {
    ...
    // Concurrent create on same mail/account target can hit UNIQUE; recover by updating the winner.
    if (!String(error).toUpperCase().includes('UNIQUE')) throw error;
    const raced = await activeShareFromRow(env, await findShareByTarget(env, input), baseUrl);
    if (!raced) throw error;
    await env.DB.prepare(`UPDATE shares SET expires_at = ?, updated_at = ? WHERE id = ?`).bind(expiry, updatedAt, raced.id).run();
    return {
      ...raced,
      expiresAt: expiry,
      updatedAt

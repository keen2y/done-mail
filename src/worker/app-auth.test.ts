...
function createKv() {
  ...
  return {
    ...
    put: async (key: string, value: string, options?: { expirationTtl?: number }) => {
      if (key.startsWith('rate:')) rateLimitWrites.push(key.split(':')[1] || key);
...
async function createEnv() {
  ...
  } as unknown as Env & { __kv: ReturnType<typeof createKv> };
...
describe('app auth split', () => {
  ...
  it('登录成功会写入 Cookie 会话并被 bootstrap 识别', async () => {
    ...
    expect(env.__kv.rateLimitWrites).toEqual([]);
  ...
  it('公开接口鉴权失败才计入公共接口限流', async () => {
    setRateLimitsForTest({ publicApi: 1 });
    const env = await createEnv();
    const headers = {
  ...
  it('公开创建共享不使用共享访问限流，旧邮件分享路径已删除', async () => {
    setRateLimitsForTest({ publicApi: 1, publicShare: 1 });
    const env = await createEnv();
    await env.KV.put(
      'config:system',
      JSON.stringify({

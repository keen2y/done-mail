...
export async function verifyAdminKeyConfig(config: AdminKeyConfig | null, adminKey: string) {
  ...
  const hash = await hashAdminKey(adminKey, config.salt);
  return timingSafeEqualString(hash, config.hash);
}
...
export async function initializeAdminKey(env: Env, adminKey: string) {
  ...
  // KV last-write-wins: if another setup raced, reject the loser.
  const stored = await readAdminKeyConfig(env);
  if (!stored || stored.version !== config.version || stored.hash !== config.hash) {
    throw new Error('系统已经初始化');
  }

  ...
  const prepare = vi.fn((sql: string) => ({
    bind: vi.fn((...params: unknown[]) => {
      const statement = {
        sql,
        params,
        first: async () => (sql.includes('SELECT id FROM mails WHERE message_id') ? null : null),
        all: async <T>() => ({
...
describe('incoming mail', () => {
  it('未配置 R2 时仍保存邮件、正文、附件信息和搜索索引，附件标记为未存储', async () => {
    ...
    // core insert + side statements (content FTS / attachments) may be separate batches
    expect(env.DB.batch).toHaveBeenCalled();
    expect((env.DB.batch as ReturnType<typeof vi.fn>).mock.calls.length).toBeGreaterThanOrEqual(1);
    ...
    expect(String(bodyStatement?.params[2])).toContain('Your invoice is ready');
    expect(String(bodyStatement?.params[3])).not.toContain('<script');
  ...
  it('删除收件箱邮件时先删 D1，再 best-effort 删 R2', async () => {
    ...
    expect(events.indexOf('batch')).toBeLessThan(events.indexOf('r2:mail/mail_1/a.txt'));
    expect(bucketDelete).toHaveBeenCalledWith('mail/mail_1/a.txt');
    expect(prepare.mock.calls.some(([sql]) => String(sql).includes('DELETE FROM mail_bodies'))).toBe(true);
  });

  it('删除收件箱邮件时 R2 失败仍完成 D1 删除', async () => {
    const { env, batch } = createDeleteEnv({ r2Fails: true, deleted: 1 });
